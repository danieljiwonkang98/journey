...
- The graph is auto-updated automatically: `graphify watch` starts on SessionStart, a Stop hook refreshes the graph after each turn if the watcher died, and git post-commit/post-checkout hooks rebuild on commits. Do not run `graphify update .` manually unless `graphify-out/graph.json` is missing.
